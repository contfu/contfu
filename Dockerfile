...
COPY --from=build /app/packages/server/ui/dist ./dist
COPY --from=build /app/packages/server/ui/server ./server
...
WORKDIR /app
COPY --from=build /app/packages/server/app/dist/server.js /app/server.js
COPY ./packages/server/db/migrations /app/migrations
HEALTHCHECK --interval=10s --timeout=5s --start-period=30s --start-interval=1s CMD [ "bun", "-e", "assert((await fetch('http://localhost:3000/health',{method:'OPTIONS'})).status === 200)" ]
