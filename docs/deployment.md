...
Contfu's **Contfu** is managed — you use it at [contfu.com](https://contfu.com) and
never host it. What you deploy is the **Contfu runtime and local store library** that holds your synchronized
...
- **Embedded runtime** — import `@contfu/contfu`, the Contfu runtime and local store library, into your own app process and query
  the Local Store in-process.

Both run the same Contfu runtime under the hood: the [Connector](./concepts.md) receives
**Sync Messages** from Contfu, applies them to a SQLite **Local Store**, and
downloads and processes **Files** and **Media** inside your boundary.

```
                    self-hosted Server                embedded
Contfu ─sync─▶ @contfu/server ─HTTP─▶ app   |  app + @contfu/contfu
                       (Local Store)                  (Local Store, in-process)
```

## Authentication key

Both shapes need the Contfu runtime's authentication key to connect to Contfu.
...
When the key is unset, the Contfu runtime does **not** connect — but the Server/embedded app
...
The image runs the Contfu runtime and exposes the HTTP query API on port `3001`. Mount a
...
| ---------------------------- | ------------------ | -------------------------------------------------------------------------- |
| `CONTFU_KEY`                 | —                  | Authentication key for Contfu (base64url). Unset → no sync, query-only.    |
...
Run the Contfu runtime and local store library inside your own process and query it directly — no network hop:
...
| ----------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `key`                   | `process.env.CONTFU_KEY` | Contfu auth key.                                                                                                                                                                      |
...
| `i18n`                  | —                        | App-level locale defaults ([Localization](./i18n.md#embedded-runtime)).                                                                                                               |
...
A Sync Message ACK confirms that the Contfu runtime accepted the item into its Local Store and queued any referenced file work. It does **not** mean that a file has downloaded, been transformed, or had its variants generated. File downloads and media processing continue asynchronously in the local media queue.

When local file work finds a missing source URL or a repairable URL failure (HTTP `401`, `403`, or `404`), the Contfu runtime automatically requests a targeted source refresh and redelivery when the connected source can provide it. Requests are coalesced per collection and survive stream reconnection. This is the normal repair path for expired or rejected Service URLs; **Reset Source State** and **Reset Target State** are operator recovery tools, not routine media repair steps.

A refresh request only asks Contfu to repair the item. It does not guarantee that the Service can issue a fresh URL, that the source pull succeeds, or that local download and processing have finished.

## File & media storage

By default, files the Contfu runtime downloads are stored **in the SQLite database**. Set
`FILE_URL` (Server) or pass a `fileStore` (embedded) to store them on the filesystem or in
S3-compatible object storage instead.

Remote file IDs are derived from the remote URL origin and path (query strings are excluded so
refreshed signed URLs remain stable). This keeps identical paths from different hosts isolated.
Older path-only IDs remain readable in stored item references; a later sync that receives the
remote URL replaces that reference with the origin-aware ID.
...
Messages are applied — Contfu never owns file storage or media processing.
