...
<div class="page-shell p-6">
  ...
  <section class="mb-8">
    ...
    <div class="border border-border bg-card p-4 line-reveal">
      <div class="space-y-1 text-sm">
        ...
        <p><span class="text-muted-foreground">files</span><span class="mx-2 text-muted-foreground">=</span><span class="text-primary font-bold text-lg">{stats.fileCount}</span> <span class="text-muted-foreground text-xs">({stats.fileCount} found &middot; {stats.downloadedCount} downloaded &middot; {stats.processedCount} processed)</span></p>
      </div>
    </div>
  </section>
  ...
  <section class="mb-8">
    ...
    {#if stats.itemCount === 0 && stats.fileCount === 0}
      <Alert.Root>
