            ...
            {#each Object.entries(data.item.props) as [key, value]}
              <div>
                ...
                <dd class="wrap-break-words">
                  {#if Array.isArray(value)}
                    <div class="flex flex-col gap-1">
                      {#each value as element}
                        {#if typeof element === "string" && fileMap.has(element)}
                          {@const file = fileMap.get(element)!}
                          <div class="mb-2">
                            {#if file.mediaType === "image"}
                              <img
                                src={`/files/${file.id}.${file.ext}`}
                                alt={key}
                  ...
                  {:else if typeof value === "string" && fileMap.has(value)}
                    ...
                    <div>
                      ...
                      {:else if file.mediaType === "video"}
                        ...
                        ></video>
                      {:else if file.mediaType === "audio"}
                        <audio src={`/files/${file.id}.${file.ext}`} controls
                        ></audio>
                      {:else}
