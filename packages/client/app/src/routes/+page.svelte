...
<div class="container mx-auto max-w-5xl p-6">
	...
	{#if hasLoadError}
		<Alert.Root class="mb-6" variant="destructive">
			...
			<Alert.Description>
				Unable to retrieve statistics from the local database. Try refreshing the page.
			</Alert.Description>
		</Alert.Root>
	{/if}
	...
	<section class="mb-8">
		...
		<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			<Card.Root>
				...
				<Card.Content>
					<p class="text-xs text-muted-foreground">
						{stats.itemCount === 1 ? "1 synced page" : `${stats.itemCount} synced pages`}
					</p>
				</Card.Content>
			</Card.Root>
			...
			<Card.Root>
				...
				<Card.Content>
					<p class="text-xs text-muted-foreground">
						{stats.collectionCount === 1 ? "1 content collection" : `${stats.collectionCount} content collections`}
					</p>
				</Card.Content>
			</Card.Root>
			...
			<Card.Root>
				...
				<Card.Content>
					<p class="text-xs text-muted-foreground">
						{stats.assetCount === 1 ? "1 media file" : `${stats.assetCount} media files`}
					</p>
				</Card.Content>
			</Card.Root>
		</div>
	...
	<section class="mb-8">
		...
		{#if stats.itemCount === 0 && stats.assetCount === 0}
			<Alert.Root>
				...
				<Alert.Description>
					Your local database is empty. Connect to a Contfu sync server to start receiving content.
				</Alert.Description>
			</Alert.Root>
		{:else}
			<Card.Root>
				...
				<Card.Content>
					<p class="text-sm text-muted-foreground">
						Your local database contains {stats.itemCount} {stats.itemCount === 1 ? "item" : "items"} across {stats.collectionCount} {stats.collectionCount === 1 ? "collection" : "collections"}, with {stats.assetCount} {stats.assetCount === 1 ? "asset" : "assets"}.
					</p>
				</Card.Content>
			</Card.Root>
		{/if}
