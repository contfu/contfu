...
describe("serializeQueryParams", () => {
  ...
  test("serializes with as JSON", () => {
    const params = serializeQueryParams({
      with: { related: { filter: "collection = :collection" } },
    });
    const parsed = JSON.parse(params.get("with")!);
    expect(parsed.related.filter).toBe("collection = :collection");
  });
...
describe("deserializeQueryParams", () => {
  ...
  test("deserializes with JSON", () => {
    const params = new URLSearchParams({
      with: JSON.stringify({ related: { filter: "collection = :collection" } }),
    });
    const options = deserializeQueryParams(params);
    expect(options.with!.related.filter).toBe("collection = :collection");
  });
