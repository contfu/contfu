...
/**
 * Get all consumer-collection joins for a specific collection with consumer names.
 */
export const listConsumerCollectionsByCollection = (userId: number, collectionId: number) =>
  Effect.gen(function* () {
    ...
    Effect.withSpan("collections.listConsumerCollectionsByCollection", { attributes: { userId } }),
  );
