...
/**
 * List all flows where the given collection is either source or target.
 */
export const listFlowsByCollection = (collectionId: number) =>
  Effect.gen(function* () {
  ...
  }).pipe(Effect.withSpan("flows.listByCollection", { attributes: { collectionId } }));
