...
describe("extractLinks", () => {
  test("extracts scalar and list reference properties into link records", () => {
    ...
    expect(result.records).toEqual([
      { kind: "internal", prop: "related", from: ["items", 1], to: ["items", 2] },
      { kind: "internal", prop: "relatedMany", from: ["items", 1], to: ["items", 3] },
      { kind: "internal", prop: "relatedMany", from: ["items", 1], to: ["items", 4] },
    ]);
  ...
  test("extracts optional references and replaces their placeholders, including tuple metadata", () => {
    ...
    const result = extractLinks(["items", 1], props, undefined, schema);
    ...
    expect(props.related).toEqual(["items", 2]);
    expect(props.relatedMany).toEqual([
      ["items", 3],
      ["items", 4],
    ]);
  });
  ...
  test("retains null or missing optional reference lists without creating links", () => {
    ...
    const result = extractLinks(["items", 1], { nullable: null }, null, schema);
    expect(result.records).toEqual([]);
  ...
  test("fails closed for an unrecognized numeric reference property", () => {
    expect(() =>
      extractLinks(["items", 1], { title: "Article", related: ["items", 2] }, undefined, {
        title: PropertyType.STRING,
      }),
  ...
  test("retains unknown non-reference metadata for forward compatibility", () => {
    const result = extractLinks(["items", 1], { title: "Article", metadata: "future" }, undefined, {
      title: PropertyType.STRING,
    });
