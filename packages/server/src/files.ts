...
import type { FileStore, MediaConvertOpts, MediaOptimizer } from "@contfu/contfu";
import { FileLoadError, fileStore as defaultFileStore, getFile, loadFile } from "@contfu/contfu";

type RouteRequest = Request & { params: Record<string, string> };

export type ServerFileOptions = {
  fileStore?: FileStore;
...
export async function getFileStore({
...
}: Pick<ServerFileOptions, "fileStore" | "importModule"> = {}): Promise<FileStore> {
  if (fileStore) {
    return fileStore;
  }
  ...
  try {
    const fileStore = (await importModule("@contfu/bun-file-store")) as {
      FileStore: new (url: string) => FileStore;
    };
    return new fileStore.FileStore(process.env.FILE_URL);
  } catch (error) {
    throw new Error(
...
export async function getMediaOptimizer({
  mediaOptimizer,
  importModule = (name) => import(name),
}: Pick<ServerFileOptions, "mediaOptimizer" | "importModule"> = {}): Promise<MediaOptimizer> {
  if (mediaOptimizer) {
