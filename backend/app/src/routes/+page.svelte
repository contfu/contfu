...
<main class="dark:bg-gray-900">
	...
	<section class="bg-gradient-to-b from-white to-gray-50 py-40 dark:from-gray-900 dark:to-gray-800">
		<div class="container mx-auto px-4">
			<div class="mx-auto max-w-4xl text-center">
				<h1 class="mb-6 text-5xl font-bold tracking-tight text-gray-900 dark:text-white">Power-up your favourite CMS</h1>
				<p class="mb-10 text-xl text-gray-600 dark:text-gray-300">
					Seamlessly synchronize and normalize data across multiple CMS platforms. One API to rule them all.
				</p>
	...
	<section id="features" class="py-20">
		<div class="container mx-auto px-4">
			<h2 class="mb-12 text-center text-3xl font-bold text-gray-900 dark:text-white">Why choose Contfu?</h2>
	...
	<section id="pricing" class="bg-gray-50 py-20 dark:bg-gray-800">
		<div class="container mx-auto px-4">
			<h2 class="mb-12 text-center text-3xl font-bold text-gray-900 dark:text-white">Simple, transparent pricing</h2>
			<p class="mb-8 text-center text-gray-600 dark:text-gray-300">All plans come with a 7-day free trial.</p>
			<div class="mb-8 flex justify-center">
				<div class="flex items-center gap-3">
					<span class={`text-sm ${!isYearly ? "text-primary-600 dark:text-primary-400 font-bold" : "text-gray-600 dark:text-gray-400"}`}>
						Monthly
					</span>
					<button
						onclick={() => (isYearly = !isYearly)}
						class={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
							isYearly ? "bg-primary-600 dark:bg-primary-500" : "bg-gray-300 dark:bg-gray-600"
						}`}
					>
						<span
			...
			<div class="grid grid-cols-1 gap-8 md:grid-cols-3">
				{#each data.products as product}
					{@const price = product.prices[isYearly ? "yearly" : "monthly"]}
					{#if price}
						<div
						...
						>
							{#if product.recommended}
								<div
									class="bg-primary-600 dark:bg-primary-500 absolute -top-4 left-0 right-0 mx-auto w-32 rounded-full py-1 text-center text-sm font-semibold text-white"
								>
									Recommended
								</div>
							{/if}
							<h3 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">{product.name}</h3>
							<div class="mb-4">
								<span class="text-4xl font-bold text-gray-900 dark:text-white">
									&euro;{isYearly ? Math.round(price.amount / 12) : price.amount}
								</span>
								<span class="text-gray-600 dark:text-gray-400">/month</span>
								{#if isYearly}
									<div class="mt-1 text-sm text-gray-500 dark:text-gray-400">Billed &euro;{price.amount}/year</div>
								{/if}
							</div>
	...
	<section class="bg-primary-400 dark:bg-primary-600 py-20">
		<div class="container mx-auto px-4 text-center">
			<h2 class="mb-6 text-3xl font-bold text-white">Ready to streamline your content management?</h2>
			<a
				href="#pricing"
