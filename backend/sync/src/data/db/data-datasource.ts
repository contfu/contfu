...
export async function createSource(
  ...
  return (
    await db.insert(sourceTable).values({ userId, id, name, url, type, credentials }).returning()
  )[0];
}
...
export async function createCollection(
  ...
  return (
    await db.insert(collectionTable).values({ userId, sourceId, name, id, ref }).returning()
  )[0];
}

export async function addItemIds(userId: number, collectionId: number, toAdd: Buffer[]) {
...
export async function createConnection(userId: number, consumerId: number, collectionId: number) {
  return (
    await db.insert(connectionTable).values({ userId, consumerId, collectionId }).returning()
  )[0];
}

export async function countCollectionsForConsumer(userId: number, consumerId: number) {
  return db.$count(
    db
...
export async function createItemIdConflictResolution(
  ...
  await db.insert(itemIdConflictResolutionTable).values({ userId, collectionId, sourceItemId, id });
}

export async function getItemId(userId: number, collectionId: number, sourceItemId: Buffer) {
