...
2. **Decide**: is this an actionable code change, or does it need discussion?
   - **Actionable** — implement the change, then reply on the PR confirming what was done.
   - **Needs discussion** — reply on the PR with a question or explanation.
...
4. **Reply** — post a comment on the PR summarizing what was done:
   ...
   ```
5. **After all comments are addressed** — run quality checks, amend the commit, and force-push:
   ```bash
